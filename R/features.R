...
add_features <- function(ohlc, window_sizes = c(5, 10, 20, 40, 80, 150, 300)) {
  ...
  n_ <- 5
  ohlc$ctc <- volatility(quantmod::OHLC(ohlc), n = n_, calc = "close")
  ohlc$parkinson <- volatility(quantmod::OHLC(ohlc), n = n_, calc = "parkinson")
  ohlc$rogers <- volatility(quantmod::OHLC(ohlc), n = n_, calc = "rogers.satchell")
  ohlc$gkyz <- volatility(quantmod::OHLC(ohlc), n = n_, calc = "gk.yz")
  ohlc$yz <- volatility(quantmod::OHLC(ohlc), n = n_, calc = "yang.zhang")
