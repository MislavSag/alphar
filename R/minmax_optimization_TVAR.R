...
X <- as.data.frame(indicators[, .(returns, sum_excess_p_999_year)])
  ...
  runner(
    ...
    f = function(x) {
      ...
      if (is.null(tv1)) {
        ...
        print(data.frame(threshold = tv1$model.specific$Thresh,predictions = data.frame(as.list(tv1_pred))))
        data.frame(threshold = tv1$model.specific$Thresh,predictions = data.frame(as.list(tv1_pred)))}, error = function(e) NULL)
...
tvar_res <- lapply(roll_results, function(x){
  cbind(indicators[1:nrow(x), .(time, returns, sum_excess_p_999_year)], x)
})
...
ggplot(tvar_res[[1]], aes(time)) +
  ...
  geom_line(aes(y = sum_excess_p_999_year))
ggplot(tvar_res[[1]][time %between% c("2021-01-01", "2022-10-01")], aes(time)) +
  geom_line(aes(y = threshold_1), color = "green") +
  geom_line(aes(y = threshold_2), color = "red") +
  geom_line(aes(y = sum_excess_p_999_year))
...
tvar_backtest <- function(tvar_res) {
  ...
  indicator <- tvar_res$sum_excess_p_999_year
...
lapply(tvar_res[1], function(x) Return.cumulative(tvar_backtest(x)))
window_lengths / 7 / 22
